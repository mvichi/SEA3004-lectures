...
\section{Tutorial 1: Scalar and vector fields}

The aim of the tutorial is to familiarize with scalar and vector fields,
i.e. arrays in geosciences. We will explore a scalar field representing
mean sea level pressure values (the field is obtained from an atmospheric
\emph{reanalysis} for the mean December 2014 conditions done by the
...
\section{Tutorial 2: mathematical fields}

Fields can also be expressed as mathematical functions, and they can
be both scalar and vector fields. Matlab allows to graph 2D fields, using the \texttt{pcolor} function for scalars and the \texttt{quiver}
...
and \texttt{/} operators between matrices are indicated by the symbols
\texttt{.{*}} and \texttt{./. }
...
>> x=-10:10;
>> y=-10:10;
>> [X,Y]=meshgrid(x,y); % this function creates 2D arrays representing the plane
>> whos
...
>> print -dpng vector_field.png
\end{lstlisting}


\section{Tutorial 2: distances on a sphere}
...
\section{Exercise 1: Plotting scalar and vector fields on a plane {[}20+10{]}}

An atmospheric feature is described by the scalar field $\theta\left(x,y\right)=283+\frac{x^{2}+y^{2}}{15}$and
by the vector field $\mathbf{F}\left(x,y\right)=2x\hat{\mathbf{i}}+2y\hat{\mathbf{j}}$
...
\begin{enumerate}
\item Plot a pseudocolor map of the scalar field with the colorbar and indicate
in the title what type of physical field this can be and the units.
...
\item Add text cells to explain what kind of atmospheric feature can be represented by these two fields
\item \emph{10 Bonus Points: create another figure with both plots overlain
(a color map with the vectors)}
\end{enumerate}
